...
public class DefaultRestUtil implements RestUtil {
    ...
    private String execute(BaseRestWorker restWorker) throws RestExecException {
        ...
        try {
            ...
            myLog.error(ie.getMessage(), ie.fillInStackTrace());
            throw new RestExecException(restWorker.getHttpMethod().getStatusCode(),
                    "InterruptedException occur");
        } catch (ExecutionException ee) {
            myLog.error(ee.getMessage(), ee.fillInStackTrace());
            throw new RestExecException(restWorker.getHttpMethod().getStatusCode(),
                    "Execution Exception occur", ee);
        } catch (Exception e) {
            e.printStackTrace();
        }
